...
BoutReal limitFreeExp(BoutReal fm, BoutReal fc) {
  BoutReal fp = fc / fm;
  return fp;
}
...
int Hermes::init(bool restarting) {
  ...
  Vort_sheathdissipation = optvort["Vort_sheathdissipation"].doc("Use dissipation of vorticity infront of sheath").withDefault<bool>(false);
  Vort_phidissipation = optvort["Vort_phidissipation"].doc("Use dissipation for parallel electric field").withDefault<bool>(false);
  ...
  Nn_numdiff = optnn["Nn_numdiff"].doc("Use neutral density source terms").withDefault<bool>(false);
  ...
  NnVn_numdiff = optnnvn["NnVn_numdiff"].doc("Use neutral momentum parallel flow").withDefault<bool>(false);
  ...
  Pn_numdiff = optpn["Pn_numdiff"].doc("Use neutral momentum parallel flow").withDefault<bool>(false);
  ...
  if (TE_Nn){
    SAVE_REPEAT(TE_Nn_parflow, TE_Nn_perpflow, TE_Nn_sources, TE_Nn_hyper, TE_Nn_numdiff);
  }

  if (TE_NnVn){
    SAVE_REPEAT(TE_NnVn_parflow, TE_NnVn_perpflow, TE_NnVn_pargradient, TE_NnVn_pardiffusion, TE_NnVn_friction, TE_NnVn_hyper, TE_NnVn_numdiff);
  }
  ...
  OPTION(optneutrals, neutral_recombination, true);
  // Density
  TE_Ne_ExB = 0.0;
  TE_Ne_mag = 0.0;
  TE_Ne_parflow = 0.0;
  ...
  if (TE_Ne) {
    ...
    if (Ne_anomalous_par){
      SAVE_REPEAT(TE_Ne_anomalous_par);
    }
  }
  ...
  if (TE_Vort) {
    ...
    if (Vort_sheathdissipation) {
      SAVE_REPEAT(TE_Vort_sheathdissipation);
    }
    if (Vort_phidissipation) {
      SAVE_REPEAT(TE_Vort_phidissipation);
    }
  }
  ...
  anomalous_D_par = opttransport["anomalous_D_par"].doc("Anomalous parallel diffusion").withDefault(0.0);
  ...
  *_FCIDiv_a_Grad_perp *= rho_s0;
  ...
  OPTION(optnumerics, newXZsolver, false);
  
    // Use older Laplacian solver                                                                                                                 
  if (newXZsolver){
    newSolver = LaplaceXZ::create(mesh,&opt["phiSolver"]);
  ...
  if (calc_potential && !steady_state){
	...
	if (newXZsolver){
	  ...
	  if (mesh->firstX()) {
            for (int j = mesh->ystart; j <= mesh->yend; j++) {
              for (int k = 0; k < mesh->LocalNz; k++) {
		phi_boundary3d(mesh->xstart  , j, k) =0.5 * ( Pi(mesh->xstart, j, k)+Pi(mesh->xstart, j, k) );
              }
            }
          }
          ...
          if (mesh->lastX()) {
            for (int j = mesh->ystart; j <= mesh->yend; j++) {
              for (int k = 0; k < mesh->LocalNz; k++) {
		phi_boundary3d(mesh->xend+1, j, k) = 0.5 * (Pi(mesh->xend+1, j, k) + Pi(mesh->xend+1, j, k));
              }
            }
          }
	  ...
	  if (mesh->firstX()) {
		...
		//phi_boundary3d(mesh->xstart - 2, j, k) = 0.5 * ( Pi(mesh->xstart - 2, j, k) + Pi(mesh->xstart - 1, j, k));
	      }
	    }
	  }	
	  ...
	  if (mesh->lastX()) {
		...
		//phi_boundary3d(mesh->xend + 2, j, k) = 0.5 * ( Pi(mesh->xend + 2, j, k) + Pi(mesh->xend +1 , j, k) );
	      }
	    }
	  }
	}
  ...
  */
  ...
  if (evolve_neutrals){
    ...
    fci_neutral_rates( Ne, Te, Ti, Vi, Nn, Tn, Vn, Sneutral, Fn, Qin, Rn, Riz, Rrc, Rcx, Tnorm, Nnorm, Bnorm, rho_s0,
		       Omega_ci, true, Dnn, neutrals_lmax, evolve_pn, neutral_average, neutral_recombination);
  ...
  if (evolve_ne){
    ...
    if (Ne_anomalous_par) {
      TE_Ne_anomalous_par = Div_par_K_Grad_par_mod(a_d3d_par, Ne);
    ...
    if (Ne_lowdiffuse){
      if (use_new_divagradperp){
	TE_Ne_lowdiffuse = (low_diffuse_value_Ne * a_d3d / Ne) * FCIDiv_a_Grad_perp(oness,Ne);
      } else {
  ...
  if (evolve_te){
	...
	TE_Pe_lowdiffuse = (low_diffuse_value_Te * a_chi3d / Te) * FCIDiv_a_Grad_perp(oness,Te);
      } else {
  ...
  if (evolve_ti){
    ...
    if (Pi_lowdiffuse){
      if (use_new_divagradperp){
	TE_Pi_lowdiffuse = (low_diffuse_value_Ti * a_chi3d / Ti) * FCIDiv_a_Grad_perp(oness,Ti);
      } else {
  ...
  if (evolve_neutrals){
	...
	TE_Nn_perpflow = Dnn * new_Delp2(Nn);
	ddt(Nn) += TE_Nn_perpflow;
      }
    } // End Nn_perpflow
