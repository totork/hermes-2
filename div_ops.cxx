...
const Field3D Div_par_mod(const Field3D& f, const Field3D& v, const Field3D& fastest, const bool& fl) {
  ...
  return result;
    ...
    BOUT_FOR(i, result.getRegion("RGN_NOBNDRY")) {
      ...
      BoutReal c = 0.5 * (f[i] + f.yup()[iyp]) * (v[i] + v.yup()[iyp]);             // K at the upper boundary                                       
      ...
      // Calculate flux at lower surface                                                                                                            
      c = 0.5 * (f[i] + f.ydown()[iym]) * (v[i] + v.ydown()[iym]);           // K at the lower boundary                                              
...
BoutReal CustomStencil::apply(const Field3D &a, const Ind3D &i) {
  ...
  for (size_t c = 0; c < coefs.size(); ++c) {
    //result += coefs[c][i] * a[i.xp(xoffset[c]).zpm(zoffset[c])]; OLD ONE, NOT WORKING WITH NEW BOUT VERSION
